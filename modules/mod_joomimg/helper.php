...
// $Id: helper.php 4126 2013-03-03 14:44:08Z erftralle $
/****************************************************************************************\
  ...
  function modContent(&$objects)
  {
    ...
    foreach($objects as $key => $obj)
    {
     ...
     // Wordwrap for image description
	  ...
	  //echo'<pre>';print_r($objects[$key]->link,0);echo'</pre>';
      ...
      if($this->getConfig('image_position') != 0)
      {
        ...
        {
          case 'thumb':
            $imgpath   = JPATH_SITE.DS.$this->getJConfig('jg_paththumbs');
            break;
          case 'img':
            $imgpath   = JPATH_SITE.DS.$this->getJConfig('jg_pathimages');
            break;
          case 'orig':
            $imgpath   = JPATH_SITE.DS.$this->getJConfig('jg_pathoriginalimages');
            break;
        }
      ...
      switch ($this->getConfig('image_position'))
      {
        ...
        case 3:
          ...
          if($this->getConfig('setjilink'))
          {
			/*
			$str = "gomel-cup-2013/";
			$count_str = mb_substr_count($obj->link, $str);
			echo $count_str." \ ";
			switch($count_str){
				case 1:
					//$str = "sorevs2013/gomel-cup-2013/";
					$obj->link = str_replace($str,'',$obj->link);
					break;
				case 2:
				default:
					$str1 = "sorevs2013/gomel-cup-2013/";
					$str2 = "sorevs2013/";
					$obj->link = str_replace($str1,$str2,$obj->link);
					break;
			}
			*/
			$objects[$key]->imgelem .= '  <a href="'.$obj->link.'" rel="nofollow">';
          }
